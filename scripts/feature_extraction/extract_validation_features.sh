...
#
# Options:
#   --csv FILE          CSV file with sequences (default: data/raw/validation_sequences.csv)
...
CSV_FILE="data/raw/validation_sequences.csv"
...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    --force)
      ...
      ;;
    --help)
      echo "Usage: ./extract_validation_features.sh [OPTIONS]"
      echo "Extract features for validation RNA data"
      echo ""
      echo "Options:"
      echo "  --csv FILE          CSV file with sequences (default: data/raw/validation_sequences.csv)"
...
PROGRESS_FILE="$OUTPUT_DIR/logs/validation_progress.json"
...
echo "=== RNA validation Feature Extraction ==="
...
def batch_process_targets(target_ids, extract_thermo=True, extract_dihedral=True, extract_mi=True):
    ...
    with open(Path(args.output_dir) / 'validation_processing_summary.json', 'w') as f:
...
def generate_html_report(output_dir):
    ...
    try:
        ...
        progress_file = os.path.join(output_dir, "logs", "validation_progress.json")
        with open(progress_file, 'r') as f:
            progress = json.load(f)
        
        # Generate simple HTML report
        report_file = os.path.join(output_dir, "reports", "validation_report.html")
        ...
        with open(report_file, 'w') as f:
            ...
            <html>
            <head>
                <title>validation Feature Extraction Report</title>
                <style>
            ...
            <body>
                <h1>validation Feature Extraction Report</h1>
...
def main():
    parser = argparse.ArgumentParser(description="Extract features for validation data")
    ...
    # For validation data, extract base IDs (without residue numbers)
...
echo "python $OUTPUT_DIR/logs/validation_extraction_script.py$SCRIPT_ARGS"

# Create a log file for the run
LOG_FILE="$OUTPUT_DIR/logs/validation_extraction_$(date +%Y%m%d_%H%M%S).log"
echo "Logging to $LOG_FILE"

# Execute the script
python "$OUTPUT_DIR/logs/validation_extraction_script.py"$SCRIPT_ARGS 2>&1 | tee "$LOG_FILE"
...
try:
    ...
    # Generate simple HTML report
    report_file = os.path.join(output_dir, 'reports', 'validation_final_report.html')
    ...
    with open(report_file, 'w') as f:
        ...
        <body>
            <h1>validation Feature Extraction Final Report</h1>
...
"
    ...
    echo "===== validation Feature Extraction Failed ====="
